...
import { User, Brain, TrendingUp, BarChart3, Menu, RefreshCw, Users, UserPlus, Home, Settings, LogOut, Briefcase, UserCheck, X, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function Dashboard() {
    ...
    const MLStatusPanel = ({ status, title, serviceType = "profit" }) => (
        <div className="bg-white/5 backdrop-blur-md p-4 md:p-6 rounded-2xl shadow-lg">
            ...
            {status ? (
                <div className={`p-3 md:p-4 rounded-lg ${status.success ? "bg-green-500/20 border-green-500" : "bg-red-500/20 border-red-500"} border`}>
                    <p className="font-semibold text-white text-sm md:text-base">Service: {status.ml_service}</p>
                    ...
                    {status.ml_info && (
                        <p className="text-green-300 text-xs md:text-sm mt-1">{status.ml_info.message}</p>
                    )}
                    {status.error && <p className="text-red-300 mt-1 text-xs md:text-sm">Error: {status.error}</p>}
                </div>
            ) : (
                <p className="text-gray-400 text-sm md:text-base">Checking status...</p>
            )}
            <button 
                onClick={serviceType === "profit" ? checkProfitMLStatus : checkCustomerMLStatus} 
                className="mt-2 text-[#A78BFA] hover:text-[#8B5CF6] flex items-center gap-1 text-sm md:text-base"
            >
    ...
    const CustomerDataCard = ({ icon: Icon, title, value, colorClass }) => (
        <div className="bg-white/5 backdrop-blur-md p-4 md:p-6 rounded-2xl shadow-lg flex flex-col items-start justify-center transition-all hover:shadow-[0_0_25px_#7C3AED]/30">
            <div className={`p-2 md:p-3 rounded-full ${colorClass} mb-3 md:mb-4`}>
    ...
    const TomorrowPredictionCard = () => (
        <div className="bg-gradient-to-br from-purple-600/20 to-pink-600/20 p-4 md:p-6 rounded-2xl shadow-lg border border-purple-500/30">
            <h3 className="text-lg md:text-xl font-semibold text-white flex items-center gap-2 mb-3 md:mb-4">
            ...
            {tomorrowPrediction ? (
                ...
                <div className="text-center">
                    <p className="text-gray-300 mb-3 text-sm md:text-base">No prediction generated yet</p>
                    <button 
                        onClick={getTomorrowPrediction} 
                        disabled={loading}
                        className="bg-[#7C3AED] px-3 py-2 md:px-4 md:py-2 rounded-lg text-white hover:bg-[#8B5CF6] disabled:opacity-50 text-sm md:text-base"
                    >
    ...
    const CustomerPredictionCard = () => (
        <div className="bg-gradient-to-br from-green-600/20 to-blue-600/20 p-4 md:p-6 rounded-2xl shadow-lg border border-green-500/30">
            <h3 className="text-lg md:text-xl font-semibold text-white flex items-center gap-2 mb-3 md:mb-4">
            ...
            {customerPrediction ? (
                ...
                <div className="text-center">
                    <p className="text-gray-300 mb-3 text-sm md:text-base">No customer prediction yet</p>
                    <button 
    ...
    return (
        <div className="flex h-screen bg-[#0F0F12] font-sans text-gray-100 overflow-hidden">
            {/* Mobile Overlay */}
            {isMobile && sidebarOpen && (
                <div 
            ...
            <aside 
                ...
                className={`
                    ...
                    bg-[#141419]/80 backdrop-blur-lg border-r border-gray-700/50
                `}
            >
                <div className="flex flex-col h-full p-4 md:p-6">
                    ...
                    <div className="flex items-center justify-between mb-6 md:mb-8">
                        <div className="flex items-center gap-3">
                            ...
                            <h1 className="text-lg md:text-xl font-bold text-white">CustomerCare Pro</h1>
                        </div>
                        {isMobile && (
                            <button
                                onClick={() => setSidebarOpen(false)}
                    ...
                    <nav className="flex-1 space-y-1 md:space-y-2">
                        <button
                            ...
                            className={`w-full flex items-center gap-3 px-3 py-2 md:px-4 md:py-3 rounded-lg transition-all ${
                        ...
                        >
                            ...
                            <span className="font-medium text-sm md:text-base">All Employees</span>
                        </button>
                        <button
                        ...
                        <button
                            ...
                            className="w-full flex items-center gap-3 px-3 py-2 md:px-4 md:py-3 rounded-lg text-gray-400 hover:bg-white/10 hover:text-white transition-all"
                        >
                    ...
                    <div className="pt-4 md:pt-6 border-t border-gray-700">
                        <div className="flex items-center gap-3 mb-3 md:mb-4">
                            <div className="w-8 h-8 md:w-10 md:h-10 bg-[#7C3AED] rounded-full flex items-center justify-center">
                                <User size={16} className="text-white md:w-5 md:h-5" />
                            </div>
                            <div className="flex-1 min-w-0">
                                <p className="text-white font-medium text-sm md:text-base truncate">Manager</p>
                                <p className="text-gray-400 text-xs md:text-sm truncate">Admin Access</p>
                            </div>
                        </div>
                        <button
                            onClick={handleLogout}
                            className="w-full flex items-center gap-3 px-3 py-2 md:px-4 md:py-3 text-gray-400 hover:bg-red-500/10 hover:text-red-400 rounded-lg transition-all text-sm md:text-base"
                        >
            ...
            <div className="flex-1 flex flex-col overflow-hidden lg:ml-72">
                {/* Mobile Header */}
                <div className="lg:hidden flex items-center justify-between p-4 bg-[#0F0F12] border-b border-gray-800">
                    <button 
                        id="hamburger-btn"
                ...
                <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 space-y-4 md:space-y-6">
                    {/* Header Controls */}
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4 md:mb-8">
                        <h2 className="text-xl md:text-2xl lg:text-3xl font-bold text-white">AI-Powered Sales & Customer Dashboard</h2>
                        <div className="flex flex-wrap gap-2">
                            <button 
                                onClick={generatePredictions} 
                                disabled={loading || !profitMlStatus?.success}
                                className="bg-[#7C3AED] px-3 py-2 md:px-4 md:py-2 rounded-lg text-white hover:bg-[#8B5CF6] shadow-lg disabled:opacity-50 flex items-center gap-2 text-sm md:text-base"
                            >
                                {loading ? "Generating..." : "AI Predictions"}
                            </button>
                            <button 
                                onClick={addSampleSalesData}
                                className="bg-green-600 px-3 py-2 md:px-4 md:py-2 rounded-lg text-white hover:bg-green-700 shadow-lg text-sm md:text-base"
                            >
                                Add Sales Data
                            </button>
                            <button 
                                onClick={addSampleCustomerData}
                                className="bg-blue-600 px-3 py-2 md:px-4 md:py-2 rounded-lg text-white hover:bg-blue-700 shadow-lg text-sm md:text-base"
                            >
                                Add Customer Data
                            </button>
                            <button 
                                ...
                                disabled={refreshing}
                                className="bg-gray-600 px-3 py-2 md:px-4 md:py-2 rounded-lg text-white hover:bg-gray-700 shadow-lg disabled:opacity-50 flex items-center gap-2 text-sm md:text-base"
                            >
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <h3 className="text-lg md:text-xl font-semibold text-white">Sales Prediction Analysis</h3>
                    ...
                    <div className="bg-white/5 backdrop-blur-md p-4 md:p-6 rounded-2xl shadow-lg">
                        {profitRecords.length > 0 ? (
                            ...
                            <div className="text-center py-6 md:py-10 text-gray-400">
                                <p className="text-base md:text-lg mb-2">No sales data available</p>
                                <p className="text-xs md:text-sm">Add sample data or connect to your database</p>
                            </div>
                        )}
                    </div>

                    {/* Data Table */}
                    <div className="bg-white/5 backdrop-blur-md p-4 md:p-6 rounded-2xl shadow-lg overflow-x-auto">
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2">
                            <h3 className="text-lg md:text-xl font-semibold text-white">Sales Records</h3>
                            ...
                            <table className="w-full text-left text-xs md:text-sm">
                                <thead>
                                    <tr className="border-b border-gray-700 text-gray-400 uppercase text-xs">
                                        <th className="py-2 px-2 md:py-3 md:px-4">Date</th>
                                        <th className="py-2 px-2 md:py-3 md:px-4 text-right">Actual Sales</th>
                                        <th className="py-2 px-2 md:py-3 md:px-4 text-right">Predicted Sales</th>
                                        <th className="py-2 px-2 md:py-3 md:px-4 text-right">Difference</th>
                                    ...
                                    {profitRecords.slice().reverse().map((record, index) => {
                                        ...
                                        return (
                                            <tr key={index} className="border-b border-gray-800 hover:bg-white/5 transition-colors">
                                                ...
                                                <td className="py-2 px-2 md:py-3 md:px-4 text-right text-green-400 font-mono text-xs md:text-sm">
                                                    {record.totalSales ? `$${record.totalSales.toFixed(2)}` : "N/A"}
                                                </td>
                                                <td className="py-2 px-2 md:py-3 md:px-4 text-right text-[#A78BFA] font-mono text-xs md:text-sm">
                                                    {record.predictedSales ? `$${record.predictedSales.toFixed(2)}` : "N/A"}
                                                </td>
                                                <td className="py-2 px-2 md:py-3 md:px-4 text-right font-mono text-xs md:text-sm">
                                                    {difference !== null ? (
